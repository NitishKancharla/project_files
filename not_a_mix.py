...
def is_valid_ctc_answer(answer):
    return bool(re.match(r"^\s*\d+(\.\d+)?\s*(lpa|lakhs?|lacks?)\s*$", answer.strip().lower()))

def is_valid_fresher_answer(answer):
    return answer.lower() in ['yes', 'no']

def is_valid_notice_period(answer):
    return bool(re.match(r"^\s*(\d+)\s*(days?|month|months?)\s*$", answer.strip().lower()))
...
def calculate_match_score(parsed_data, user_answers):
    score = 0
    max_score = 100
    ...
    if parsed_data["candidate_location"].lower() == parsed_data["job_location"].lower():
        score += 20
    elif "yes" in user_answers[0].lower():
        score += 20  # partial for relocation willingness

    # Fresher Match (20%)
    if (parsed_data["appears_fresher"] and user_answers[1].lower() == "yes") or \
       (not parsed_data["appears_fresher"] and user_answers[1].lower() == "no"):
        score += 20

    # Skills Proficiency Match (up to 60%)
    skills = parsed_data["skills_from_jd"]
    skill_scores = 0
    count = 0
    skill_answers = user_answers[2:]
    for i in range(0, len(skills)*2, 2):
        if i+1 < len(skill_answers):
            try:
                proficiency = int(skill_answers[i])
                years = int(skill_answers[i+1])
                count += 1
                skill_scores += min(proficiency * 2, 20)  # max 20 per skill
            except:
                pass
    if count > 0:
        score += min(skill_scores, 60)
...
if st.session_state['initialized']:
        ...
        if st.button("Submit", key=f"submit_{st.session_state['step']}"):
            if answer:
                if "ctc" in current_question.lower() and not is_valid_ctc_answer(answer):
                    st.warning("❗ Please enter a valid CTC (e.g., 6 LPA).")
    ...
    # After all Qs answered
    if st.session_state['step'] >= len(st.session_state['questions']):
        st.markdown("### 🎯 Final Summary & Match Score")
        ...
        score = calculate_match_score(st.session_state['parsed_data'], st.session_state['answers'])
        st.subheader("📊 Match Score")
        st.progress(score / 100)
        st.markdown(f"### ✅ Your profile matches **{score}%** of the job requirements!")
