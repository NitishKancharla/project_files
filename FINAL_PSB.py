            ...
            for i in range(0, len(skills)*2, 2):
                if i+1 < len(skill_answers):
                    try:
                        ...
                        skill_scores += proficiency * years / 100
                        count += 1
                    except ValueError:
                        continue
            if count > 0:
                skill_scores = skill_scores / count
                score += skill_scores

            return score

        user_answers_copy = st.session_state['answers']
        score = calculate_match_score(st.session_state['parsed_data'], user_answers_copy)
